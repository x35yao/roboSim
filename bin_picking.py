...
meshes['bolt'] = Mesh(urdf_path="meshes/urdfs/bolt.urdf",
                      ...
                      num_copies= num_copies,
                      randomize_pose = True)
# Diameter of the bowl is 0.8. The object coordinate origin of the bow is not at the center of the bowl. The center of the bowl is at (-0.15, 0)
#wrt to the object center of the bowl.
meshes['bin_origin'] = Mesh(urdf_path="/meshes/urdfs/bowl/bowl.urdf",
                     mesh_pos=[0.6, 0, 0],
                     mesh_orn=BASE_ORN,
                     orig_axis=BASE_NORM,
                     is_fixed=True,
                     num_copies=1,
                     randomize_pose=False)
meshes['bin_target'] = Mesh(urdf_path="/meshes/urdfs/bowl/bowl.urdf",
                     mesh_pos=[-0.35, 0, -0.5],
                     mesh_orn=BASE_ORN,
                     orig_axis=BASE_NORM,
                     is_fixed=True,
...
def main():
  ...
  while (1):
    # continue
    robot.step(automate_clean = True)
    time.sleep(timeStep)
